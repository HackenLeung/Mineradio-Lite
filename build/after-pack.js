...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for Mineradio Lite icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'Mineradio Lite';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`Mineradio Lite executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`Mineradio Lite icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting Mineradio Lite resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'Mineradio Lite',
    '--set-version-string', 'ProductName', 'Mineradio Lite',
    '--set-version-string', 'CompanyName', 'Mineradio Lite',
